...
def normalize_config(value):
    """Ensure config value is a string, even if pytest returns a list."""
...
def browser_context(request):
    browser_name = normalize_config(get_config_value(request.config, "browser"))
    headed_flag = get_config_value(request.config, "headed")
    video_option = normalize_config(get_config_value(request.config, "video"))
    ...
    # Create context with video if configured
...
def page(request, browser_context):
    base_url = normalize_config(get_config_value(request.config, "base_url"))
    screenshot_option = normalize_config(get_config_value(request.config, "screenshot"))
    tracing_option = normalize_config(get_config_value(request.config, "tracing"))
    video_option = normalize_config(get_config_value(request.config, "video"))
    ...
    # After test: save artifacts if test failed
